...
namespace Code.Gameplay.Character.Features
{
    public class Movement : Feature
    {
        ...
        public override void InitializeFeature(Controller controller) //TODO Add Stun Check
        {
            ...
            _dependencies.TryGet(out jump);
            _dependencies.TryGet(out _sprint);
        }
        ...
        private void Move(Vector3 moveDirection)
        {
            ...
            if(physics.OnSlope && !jump.OnDeparture)
            {
                ...
                _invoker.AddForce.Execute(new (direction, 20f * _sprint.Acceleration, ForceMode.Acceleration));
            ...
            _invoker.AddForce.Execute(new AddForceParams(direction.normalized, 10f * (physics.OnGround ? 1f : _airMultiplier) * _sprint.Acceleration, ForceMode.Acceleration));
        }
        ...
        private void LimitSpeed()
        {
            ...
            if(physics.OnSlope && !jump.OnDeparture)
            {
                if(velocity.magnitude > _sprint.MaxSpeed)
                    _invoker.Velocity.Execute(velocity.normalized * _sprint.MaxSpeed);
            ...
            if(flatVelocity.magnitude > _sprint.MaxSpeed)
            {
                flatVelocity = flatVelocity.normalized * _sprint.MaxSpeed;
                _invoker.Velocity.Execute(new (flatVelocity.x, velocity.y, flatVelocity.z));
            }
        }
    
        private void Friction(Vector3 moveDirection)
        {
